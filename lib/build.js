...
module.exports = async function (options) {
  ...
  if (verifySemver) {
    log('已开启严格验证semver版本号模式');
    if (options.appEnv !== 'local') {
      const reqUrl = `//${options.appDomain}${options.appName}/project.json`;
          ...
          if (err) {
            ...
            if (res.statusCode === 200) {
              ...
              if (!semver.valid(data.version) || !semver.valid(pkgJson.version)) {
                error('当前打包或远程版本号semver格式不正确，请检查');
                resolve(false);
              }
              if (semver.lte(pkgJson.version, data.version)) {
                error('当前打包版本号小于或等于远程版本号，请检查');
                resolve(false);
              } else {
                log(`当前打包版本号${pkgJson} > 远程版本号${data.version}，符合build条件`);
                resolve(true);
              }
              ...
              error('请求semver版本号信息返回失败', res.statusCode);
              resolve(false);
            }
          }
        });
